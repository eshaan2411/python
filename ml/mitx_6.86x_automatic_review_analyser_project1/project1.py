...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    loss_hinge = label * (np.matmul(theta, feature_vector) + theta_0)
    hinge = np.max([0.0, 1 - loss_hinge])
...
def pegasos_single_step_update(
    ...
    if label * (np.matmul(current_theta, feature_vector) + current_theta_0) <= 1:
        ...
        return (current_theta, current_theta_0 + (eta * label))
    else:
...
def bag_of_words(texts):
    ...
    """
    #load the file stopwords.txt, remove the words in the file from your dictionary, and use features
    # constructed from the new dictionary to train your model and make predictions.
    stopwords  = []
    ...
    for text in texts:
        ...
        for word in word_list:
            if word not in dictionary:
                if word not in stopwords:   # Remove to add stopwords to the dictionary.
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                # You can compute the feature vector of a word and use its count in each 
                # document rather than a binary indicator, i.e., feature_matrix[i, dictionary[word]] += 1
